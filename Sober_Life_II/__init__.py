...
font = pygame.font.Font(os.path.join(os.path.dirname(__file__), "assets", "FreeSans.ttf"), 40)
        ...
        while countery < 3:
            ...
            if gridvalue > 0:
                ...
                while counters < gridvalue:
                    # prints stressed faces
                    screen.blit(stressface, (counterx*150+35 +
                                             counters*50, countery*200+145))
                    counters = counters + 1
            countery = countery + 1
        counterx = counterx + 1
    ...
    while funcx < 9:
        ...
        while funcy < 3:
            if funcgrid[funcx][funcy] > 0:
                ...
                # type of stress in space
                gridval = int(funcgrid[funcx][funcy]/10)
    ...
    while circlecounter < 6:
        ...
        if xtemp >= 0 and xtemp < 9 and ytemp >= 0 and ytemp < 3:  # checked space on board?
            if funcgrid2[xtemp][ytemp] > 0:  # checked space contains stressor
                ...
                if thing1 == thing2:
                    # double stress if matches
                    addstress = addstress + 2*int(thing1/10)
                else:
...
async def instructions():
    ...
    yesno = pygame.image.load(os.path.join(os.path.dirname(__file__), "assets", "yesno.png"))
    line = []
    line.append(
                ...
                if ypos > 500 and ypos < 700:
                    if xpos > 750 and xpos < 1050:  # no instructions
                        ...
                        while linecounter < 10:
                            textnow = font.render(
                                line[linecounter], True, (100, 100, 100))
...
async def difficultysetting():
    screen.fill((100, 200, 200))
    stressbuttons = pygame.image.load(os.path.join(os.path.dirname(__file__), "assets", "stressbuttons.png"))
    while True:
